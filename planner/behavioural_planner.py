...
class BehaviouralPlanner:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
           arc_length += euclideanDist(waypoints[wp_index][0], waypoints[wp_index][1], waypoints[wp_index+1][0], waypoints[wp_index+1][1])
           if arc_length > self._lookahead:
               break
           wp_index += 1
        # ------------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        waypoint_local = global2local(waypoints[i], ego_state) # Converting waypoints to vehicle body reference frame
        # Any waypoint that has -ve x-coordinate lies behind the ego vehicle and hence can be neglected
        if waypoint_local[0] < 0:
            continue
        else:
            ego_to_waypoint = euclideanDist(waypoint_local[0], waypoint_local[1], 0, 0)
            if ego_to_waypoint < closest_len:
                closest_len = ego_to_waypoint
                closest_index = i
    # ------------------------------------------------------------------
    return closest_len, closest_index

# Transform coordinates from global frame to ego vehicle body reference frame
