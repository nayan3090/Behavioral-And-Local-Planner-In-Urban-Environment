...
from behavioural_planner import euclideanDist
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                score = 0
                path = paths[i]
                for point in range(len(path[0])):
                    score += euclideanDist(path[0][point], path[1][point], goal_state[0], goal_state[1])
                
                # Calculate the score based on average euclidean distance from the goal state
                score /= len(path[0])
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            # --------------------------------------------------
                            col_path = paths[j]
                            for point in range(len(col_path[0])):
                                col_score += euclideanDist(path[0][point], path[1][point], col_path[0][point], col_path[1][point])
                            
                            # Calculate the col_score based on average euclidean distance between the current non-collision path and each collision paths
                            col_score /= len(col_path[0])
                            score += self._weight * col_score
                            # # --------------------------------------------------
